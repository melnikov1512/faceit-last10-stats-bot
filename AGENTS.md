...
- `src/services/subscriptionService.js`: Match-start and match-finish event handling and notification dispatch. `handleMatchEvent` for start (stores `message_id` via `storeActiveMatch`), `handleMatchFinishedEvent` for finish (always sends a new `sendPhoto` message, replying to the start message via `replyToMessageId` when its `message_id` is known, falling back to a plain send otherwise — the start message is never modified).
...
- `src/services/telegramService.js`: Telegram Bot API integration for push notifications. `sendMessage(chatId, text, replyMarkup?)` supports optional inline keyboards. `sendPhoto(chatId, imageBuffer, caption?, replyMarkup?, replyToMessageId?)` sends a PNG via multipart/form-data, optionally as a reply via Telegram `reply_parameters`. Also exports `fetchBotUsername()` and `setMyCommands()` (both called at startup from `index.js`).
...
| `tests/unit/storageService.test.js` | `markNotificationSent` / `markFinishNotificationSentForChat` — atomic `create()` contract; `expireAt` TTL field (7 days ±5 s); `getRecentMatchIdsForPlayers` regression; `storeActiveMatch` with/without `messageId`; `getActiveMatchMessageId` hit/miss/missing-field |
| `tests/unit/telegramService.test.js` | `sendPhoto` — appends `reply_parameters` (`message_id` + `allow_sending_without_reply: true`) when `replyToMessageId` is passed; omits it when `replyToMessageId` is omitted or explicitly `null` |
| `tests/unit/subscriptionService.test.js` | `handleMatchFinishedEvent` — sends a reply (`replyToMessageId` set) when the start message's `message_id` is known; sends a plain (non-reply) message when it isn't; reads `getActiveMatchMessageId` before `removeActiveMatch` (call ordering) |
