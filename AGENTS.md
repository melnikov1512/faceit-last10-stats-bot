...
- **Image Module**: `src/services/imageService.js`. Generates FACEIT-styled PNGs using `@napi-rs/canvas`. Exports:
    ...
    - **Output**: Sorted by **hybrid rule**: players with `avg_faceit_rating` come first (sorted by rating descending); players without rating (API miss / not yet computed) are appended at the end, sorted by ADR descending. Formatted as an HTML-escaped table: `Name | Rating | ADR | K/D | Kills | ELO | ±ELO` (when rating available) or `Name | ADR | K/D | Kills | ELO | ±ELO` (fallback).
