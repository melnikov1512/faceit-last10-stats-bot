...
- **Storage Module**: `src/services/storageService.js`. Manages per-chat data using Firestore.
    - **Collections**:
        ...
        - `active_matches` — Document ID = `{chatId}_{matchId}`. Structure: `{ chatId, matchId, startedAt, messageId? }`. `messageId` (Telegram message_id) is stored when the start notification is sent; used by finish handler to edit the message. Written when a match-start notification is sent; deleted when the match finishes/is cancelled. Functions: `storeActiveMatch`, `getActiveMatchIds`, `getActiveMatchMessageId`, `removeActiveMatch`.
...
- **Telegram Module**: `src/services/telegramService.js`. Sends messages to Telegram chats via Bot API (used for push notifications from FACEIT webhook events, not the webhook reply mechanism). Exports:
    ...
    - `sendPhoto(chatId, imageBuffer, caption?, replyMarkup?)` — multipart/form-data upload; caption always uses `HTML` parse mode. **Returns** the Telegram `Message` object (contains `message_id`).
    - `editMessageMedia(chatId, messageId, imageBuffer, caption?, replyMarkup?)` — edits the photo of a previously sent message via `editMessageMedia` Bot API call. Used by finish notifications to replace the start card in-place.
...
- `src/services/subscriptionService.js`: Match-start and match-finish event handling and notification dispatch. `handleMatchEvent` for start (stores `message_id` via `storeActiveMatch`), `handleMatchFinishedEvent` for finish (edits start message via `editMessageMedia`, falls back to `sendPhoto`).
