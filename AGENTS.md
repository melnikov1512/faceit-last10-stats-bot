...
- **Image Module**: `src/services/imageService.js`. Generates FACEIT-styled PNGs using `@napi-rs/canvas`. Exports:
    ...
    - `computeMatchRating({ kills, deaths, assists, adr, teamScore, opponentScore }) → number|null` — private. Estimates a per-match Rating for result cards (rounds = teamScore + opponentScore) via `ratingEstimator.estimateMatchRating`; returns `null` if either score is missing.
    ...
    - **Estimated Rating**: `estimateAverageRating(statsArray)` maps each official FACEIT v4 per-match stats object (`Kills`, `Deaths`, `Assists`, `Rounds`, `ADR`) through `estimateMatchRating()` from `ratingEstimator.js`, then averages the non-null per-match estimates. The result is exposed on each leaderboard row as `estimated_rating` (`number|null`). This is an HLTV Rating 2.0 approximation, not FACEIT's real proprietary Rating.
...
- `src/services/ratingEstimator.js`: Pure standalone HLTV Rating 2.0 approximation helper. Exports `DEFAULT_KAST_ESTIMATE` (= 73.0 community-average fallback because FACEIT v4 does not expose KAST) and `estimateMatchRating({ kills, deaths, assists, rounds, adr, kast })`. Wired into `faceitService.getLeaderboardStats()` via `estimateAverageRating()` as the sole runtime Rating source.
