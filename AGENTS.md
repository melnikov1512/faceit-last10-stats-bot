...
- **Image Module**: `src/services/imageService.js`. Generates FACEIT-styled PNGs using `@napi-rs/canvas`. Exports:
    ...
    - **Batching**: Processes player lookups in chunks of 10 to manage API rate limits. For each player, `getPlayerInfoById`, game stats, ELO timeline, and FACEIT Rating are fetched in parallel (4 concurrent requests per player).
    - **FACEIT Rating source**: Fetched via the unofficial `match-rounds` endpoint (`https://www.faceit.com/api/statistics/v1/cs2/players/{id}/match-rounds?game_mode=5v5`) — one request per player returning all recent map rounds with `faceitRating`. Groups rounds by `matchId` and averages across maps (bo3/bo5 support). Replaces the old per-match `scoreboard-summary` approach (was N requests per player). Implemented in `getPlayerMatchRoundRatings(playerId, limit)`. **Round limit**: `Math.min(matchesCount * 5, 300)` — uses `*5` multiplier to cover worst-case bo5 series (was `*3` which caused misses for bo5 and large N values). **Cloudflare note**: This endpoint returns HTTP 403 when called concurrently from the same server IP. To avoid this, `getLeaderboardStats` fetches match-rounds for all players **sequentially** (one at a time, Step 1) before fetching the rest of the player data in parallel via FACEIT v4 API (Step 2). The pre-fetched `Map<matchId, rating>` is passed into `getPlayerStats` as the `preloadedRatings` parameter.
    - **Output**: Sorted by **hybrid rule**: players with `avg_faceit_rating` come first (sorted by rating descending, ADR descending as tiebreaker when ratings are equal); players without rating (API miss / not yet computed) are appended at the end, sorted by ADR descending. Formatted as an HTML-escaped table: `Name | Rating | ADR | K/D | Kills | ELO | ±ELO` (when rating available) or `Name | ADR | K/D | Kills | ELO | ±ELO` (fallback). Unit-tested in `tests/unit/getLeaderboardStats.test.js`.
    - **Sandbox note**: the entire `faceit.com`/`api.faceit.com` domain (Cloudflare-protected, used by the ELO timeline and match-rounds endpoints) returns HTTP 403 for *any* request — even the homepage — from some sandbox/dev network egress IPs, regardless of headers/TLS client. This is an IP-reputation block unrelated to the code; the official `open.faceit.com` Data API v4 (Bearer token) is unaffected. Verify unofficial-endpoint behavior from the actual deployed environment (GCF), not from such sandboxes.
    - **⚠️ KNOWN BROKEN IN PRODUCTION (2026-07-28)**: `getPlayerMatchRoundRatings` (match-rounds) and `getMatchScoreboardRatings` (scoreboard-summary) — both under `www.faceit.com/api/statistics/v1/cs2/...` — are confirmed **HTTP 403 for every request from Cloud Run** (production log: `[match-rounds] HTTP 403 for player {id} — skipping Rating`), not just from dev sandboxes. Cloudflare appears to hard-block the entire `www.faceit.com` domain for Google Cloud egress IP ranges (datacenter IP reputation, not a rate-limit — no amount of header/User-Agent tweaking fixes it). Verified by testing 4 different unofficial endpoint variants (player match-rounds, match scoreboard-summary, public player-history HTML page, match-room HTML page) — all 403. The **official** Data API v4 does **not** expose any Rating field either (checked live `/players/{id}/games/cs2/stats` and `/matches/{id}/stats` responses — no `Rating`/`Player Rating 2.0` key present, and the current `open.faceit.com/data/v4/docs/swagger.json` spec, identical to `ai-files/faceit-open-api.json`, only defines `additionalProperties: {}` maps with no documented Rating field). **Current behavior**: the code's existing graceful degradation (`avg_faceit_rating = null` for everyone) kicks in automatically — `/stats` silently falls back to pure-ADR sorting with no Rating column shown, no crashes. **Decision (per user, 2026-07-28)**: leave as-is for now (fallback is acceptable); revisit later. If picked up again, options discussed were: (a) compute a self-derived approximate rating from official-API-only fields (HLTV Rating 1.0-style formula using K/R, D/R, ADR, Impact — labelled clearly as an estimate, not FACEIT's real number), (b) drop the Rating column/sort entirely, or (c) pursue a proxy/anti-bot-bypass solution (costly, fragile, possible ToS concerns) — not pursued.
...
| `tests/unit/getLeaderboardStats.test.js` | `getLeaderboardStats` hybrid sort — rating DESC with ADR tiebreak, unrated players appended sorted by ADR DESC, pure-ADR fallback when no ratings, `faceit_rating_matches` count with partial coverage |
