...
- **Image Module**: `src/services/imageService.js`. Generates FACEIT-styled PNGs using `@napi-rs/canvas`. Exports:
    ...
    - **Design tokens**: cards use a glassmorphism palette in `imageService.js`, aligned with `public/index.html` CSS variables — `pageBg:#0A0A10`, `bg:rgba(255,255,255,0.07)`, `headerBg:rgba(255,255,255,0.10)`, `separator:rgba(255,255,255,0.18)`, `positive:#6EE787`, `negative:#FF6B6B`, `accent:#FF7A33`, plus mesh blob colours `meshOrange`/`meshTeal`/`meshViolet` and chip tint/border colours `positiveBorder`/`positiveTint`/`negativeBorder`/`negativeTint`. Skill-badge colours (1–10) match the web-app skill-bar segments exactly (grey / green / gold / orange / brand-orange). Every card is painted over a shared colourful "mesh" backdrop and rendered as frosted-glass panels with pill-shaped signed-value chips — see `drawMesh`/`drawGlassPanel`/`drawGlassChip`/`renderAsGlassCard` in `imageService.js`; `public/index.html` was updated in parallel to visually match this palette.
